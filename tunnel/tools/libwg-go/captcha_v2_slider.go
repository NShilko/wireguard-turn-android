...
func (s *captchaV2Session) solveSliderCaptcha(
	...
	turnLog("[КАПЧА] v2 slider puzzle decoded: grid=%d attempts=%d swaps=%d", puzzle.Size, puzzle.Attempts, len(puzzle.Swaps))
	...
	turnLog("[КАПЧА] v2 slider guesses ranked: total=%d limit=%d", len(guesses), limit)
	...
	for i := 0; i < limit; i++ {
		turnLog("[КАПЧА] v2 slider attempt %d/%d (guess #%d)", i+1, limit, guesses[i].Index)
		...
		if strings.EqualFold(check.Status, "ok") {
			...
			turnLog("[КАПЧА] v2 slider accepted on attempt %d", i+1)
...
func splitSliderStepsV2(steps []int) (int, []int, int, error) {
	...
	if len(tail)%2 != 0 {
		...
		turnLog("[КАПЧА] v2 slider payload had odd-length tail; fallback attempts=%d", attempts)
	}
	if attempts <= 0 {
		attempts = 4
	}
...
func pixelDiffV2(a, b color.Color) int64 {
	ar, ag, ab, _ := a.RGBA()
	br, bg, bb, _ := b.RGBA()
